...
class ActionableArchPackageOCITest(RiftPackageTestCase):
    ...
    """
    def setup_package(self, src_top_dir=None):
        self.make_pkg(src_top_dir=src_top_dir)
    ...
    def test_build_multiple_sources_with_main_source(self, mock_container_runtime):
        ...
        another_source_path = os.path.join(os.path.dirname(self.pkgsrc['pkg']), 'another-source')
    ...
    def test_build_unable_determine_source(self, mock_container_runtime):
        self.setup_package()
        # create another source to introduce conflict
        another_source_path = os.path.join(os.path.dirname(self.pkgsrc['pkg']), 'pkg-1.0.tar.bz2')
        with open(another_source_path, 'w+') as fh:
            fh.write('dummy')
        self.pkg.package.load()
        with self.assertRaisesRegex(
