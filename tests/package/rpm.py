...
from ..TestUtils import make_temp_file, gen_rpm_spec

class PackageRPMTest(RiftPackageTestCase):
    """
...
package:
  ...
  - pkg-debuginfos
    ...
    @patch("rift.package.rpm.Spec.check")
    @patch("rift.package.Package.check")
    def test_check(self, mock_package_check, mock_spec_check):
...
package:
  ...
  origin: Company
        ...
        pkg.load(infopath = pkgfile.name)
        pkg.check()
        # Check parent Package.check() and Spec.spec() are called.
        mock_package_check.assert_called_once()
        mock_spec_check.assert_called_once()
...
class ActionableArchPackageRPMTest(RiftPackageTestCase):
    """
    ...
    def setUp(self):
        self.init_config()
        self.pkgname = 'pkg'
        _pkg = PackageRPM(self.pkgname, self.config, self.staff, self.modules)
        self.spec_file = make_temp_file(
            gen_rpm_spec(
                name=self.pkgname,
                version="1.0",
                release="1",
...
package:
  ...
  origin: Company
        """)
        _pkg.buildfile = self.spec_file.name
    ...
    def test_test_noquit(self, mock_vm):
        ...
        mock_vm_obj.running.return_value = False
        mock_vm_obj.run_test.return_value = RunResult(0, None, None)
        results = self.pkg.test(noquit=True)
    ...
    def test_publish_staging_repo(self, mock_mock_publish):
        ...
        mock_repository = Mock()
        self.pkg.staging = mock_repository
